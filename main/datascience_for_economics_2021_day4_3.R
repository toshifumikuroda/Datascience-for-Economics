...
table1 <-  gtsummary::tbl_merge(   
  ...
  tab_spanner = c("Whole world", "Base sample", "0 - 25%", "20 - 50%", "50 - 75%", "75 - 100%"))

table1

table1 %>%
    gtsummary::as_hux_table() %>%
    huxtable::quick_xlsx(file = "./figuretable/AJR_table1.xlsx")
...
figure2 <- table2_df %>% dplyr::filter(baseco==1) %>% 
  ...
  annotate("text",label = paste0("made by ", author), x=-Inf, y=-Inf, hjust=0,vjust=0) +
  ylim(4, 10)
ggsave(file = "figuretable/AJR_figure2.png", plot = figure2, dpi = 100, width = 9.6, height = 6.4)

# Figure 3
table3_df <- foreign::read.dta("./data/AJR/maketable3.dta") %>%
...
table3_a <- huxtable::huxreg(table3_col_odd[[1]], table3_col_even[[1]],
                             ...
                             stars = NULL,
                             number_format = 2) %>%
  huxtable::add_rows(huxtable::hux("Panel A", "Dependent Variable Is Average Protection Against Expropriation Risk in 1985 to 1995",
                                   "", "", "", "", "", "", "", "", ""), after = 1) %>%
...
cat(huxtable::to_html(table3_a), 
    file = "figuretable/AJR_table3_a.html")
...
cat(huxtable::to_html(table3_b), 
    file = "figuretable/AJR_table3_b.html")
...
cat(huxtable::to_html(table4_a), 
    file = "figuretable/AJR_table4_a.html")
...
table4_b <- huxtable::huxreg("Base sample" = table4_b_col1, table4_b_col2, 
                             ...
                             coefs = c("Log European settler mortality" = "logem4",
                                       ...
                                       "Other continent dummy" = "other_cont"),
                             omit_coefs = c("(Intercept)"),
                             statistics = c('R2'= 'r.squared',"F" = "statistic"),
                             stars = NULL,
                             number_format = 2) %>%
  huxtable::merge_cells(1, 2:3) %>%
  huxtable::merge_cells(1, 4:5) %>%
...
cat(huxtable::to_html(table4_b), 
    file = "figuretable/AJR_table4_b.html")
...
table4_c <- huxtable::huxreg("Base sample" = table4_c_col1, table4_c_col2, 
                             ...
                             coefs = c("Average protection against\n expropriation risk 1985 ~ 1995" = "avexpr"),
                             omit_coefs = c("(Intercept)", "lat_abst", "asia", "africa", "other_cont"),
...
cat(huxtable::to_html(table4_c), 
    file = "figuretable/AJR_table4_c.html")
